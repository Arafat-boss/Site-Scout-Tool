              ...
              {filteredLeads.map((lead) => (
                <tr key={lead.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors group">
                  ...
                  <td className="py-4 px-4 font-medium text-zinc-900 dark:text-white max-w-[200px]">
                    ...
                    {lead.rating && (
                      <div className="flex items-center gap-1 text-[11px] text-zinc-700 dark:text-zinc-300 mt-1 font-mono">
                        <Star className="w-3 h-3 fill-zinc-700 dark:fill-zinc-300 text-zinc-700 dark:text-zinc-300" />
                        <span>{lead.rating}</span>
                        {lead.reviewsCount && <span className="text-zinc-400 dark:text-zinc-500">({lead.reviewsCount})</span>}
                      </div>
                    )}
                  </td>
                  ...
                  <td className="py-4 px-4">
                    <a
                      ...
                      className="text-zinc-700 dark:text-zinc-300 hover:text-black dark:hover:text-white hover:underline flex items-center gap-1 font-mono text-[11px]"
                    >
                  ...
                  <td className="py-4 px-4 text-[11px] space-y-1">
                    ...
                    {lead.emails.length > 0 ? (
                      <div className="flex items-center gap-1.5 text-zinc-700 dark:text-zinc-300">
                        <Mail className="w-3 h-3 text-zinc-500 dark:text-zinc-400 shrink-0" />
                        <span className="truncate max-w-[140px] font-mono">{lead.emails[0]}</span>
                      </div>
                    ) : null}
                    {(!lead.phone || lead.phone === "Not found") && lead.emails.length === 0 && (
                      <span className="text-zinc-400 dark:text-zinc-500 italic">No direct contact info</span>
                    )}
                  </td>
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  ...
                  {lead.rating && (
                    <div className="flex items-center gap-1 text-xs text-zinc-700 dark:text-zinc-300 font-mono">
                      <Star className="w-3.5 h-3.5 fill-zinc-700 dark:fill-zinc-300 text-zinc-700 dark:text-zinc-300" />
                      <span>{lead.rating}</span>
                    </div>
                  )}
                </div>

                <h4 className="text-sm font-bold text-zinc-900 dark:text-white mt-3 line-clamp-1">{lead.name}</h4>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5 line-clamp-1">{lead.address}</p>

                <a
                  href={lead.website}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-2 inline-flex items-center gap-1 text-xs font-mono text-zinc-700 dark:text-zinc-300 hover:text-black dark:hover:text-white hover:underline"
                >
              ...
              <div className="pt-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between gap-2">
                <div className="text-xs text-zinc-600 dark:text-zinc-400 font-mono truncate max-w-[140px]">
